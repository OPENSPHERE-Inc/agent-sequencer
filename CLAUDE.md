...
|----------|---------|---------|
| `.github/workflows/ci.yml` | push / PR to `master` / `main` | (a) `pytest` matrix on Linux × macOS × Windows × Python 3.11 / 3.12 / 3.13 ; (b) `uv tool install .` smoke test that validates the wheel is consumable by `uvx --from git+...` |
