    ...
    with open(jsonl_path, encoding="utf-8") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            try:
                ...
                print(
                    f"warning: {jsonl_path}:{lineno}: skipping invalid JSON ({e})",
                    file=sys.stderr,
                )
                continue
            missing = [k for k in ("id", "field", "value") if k not in event]
            if missing:
                print(
                    f"warning: {jsonl_path}:{lineno}: skipping event missing keys {missing}",
                    file=sys.stderr,
                )
                continue
