...
| ファイル | 名前 | 概要 |
|---|---|---|
| `review_rounds.py` | `review-rounds` | シーケンサプログラムを 3 体の専門家エージェント（python-sensei / sequencer-sensei / prompt-sensei）でレビューし、対応 → 検証を最大 N ラウンド繰り返して収束させる |

`review_rounds.py` が参照するスキル / エージェント / スクリプトは、隣接する
`review_rounds/` ディレクトリに自己完結しています。詳細は
[`README.md`](review_rounds/README.md) を参照してください。
