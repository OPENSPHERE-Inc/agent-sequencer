...
## 6. Determinism constraints (most important)
...
### 6.3 Aggregate with pure computation
...
## 9. Bundling programs (recommended)
...
## 10. Tips for writing prompts (Instruction.text)

Lessons learned from the `review_rounds.py` implementation:

### 10.1 Template constants + `format()`
...
Reference: during agent-sequencer development, `test_step8.py` was written in this style
(see the test file just before commit `30facb9` in the history).
