...
このドキュメントは、agent-sequencer の **シーケンサプログラム**（Python ジェネレータとして書かれた古典プログラム）を作成・変更するすべての人のためのガイドです。
...
| 順 | パス | 用途 |
|---|---|---|
| 1 | `$AGENT_SEQUENCER_PROGRAMS_DIR` | 環境変数による上書き（プラグインが同梱プログラムを公開する手段。開発時のオーバーライドにも使える） |
| 2 | `<cwd>/.claude/sequencer/programs/` | プロジェクト固有のプログラム（利用するプロジェクトと一緒にコミットする） |
| 3 | `~/.claude/sequencer/programs/` | ユーザ全体のプログラム（開くプロジェクトに関わらず利用できる） |
...
Python ジェネレータの **双方向通信** を利用します。`yield` 式は値を送出すると同時に値を受け取ります。
...
**明示的に `yield Done()` を書くことを推奨します**（可観測性のため、また JSONL ログでの追跡を容易にするため）。
...
- **やってはいけないことを明記する**（例: 理解のために他のコンテキストを参照するのは可だが、対象外の事項についてコメントするのは禁止）
...
- `additionalProperties: True` を明示的に設定する（エージェントが理由付けや統計などを含められるようにし、
...
通常は `"retry"` で十分です。「不正な値が来たら挙動を切り替える」というプログラム側のフォールバックを書くより、
スキーマで縛ってエージェントに修正させる方が堅牢です。

### 5.4 `timeout_minutes`

エージェントへの目安時間です。ドライバが強制中断することはありませんが、値は `current.last_yield` に公開されるため、
...
- **Exception**: 想定外のバグ。状態は `failed` となり、`error` 種別の `last_yield` が自動的にエージェントに返される。

## 9. ホットリロードの注意点

`agent-sequencer --watch` で起動すると、`programs/*.py` の変更時に自動でリロード（レジストリ再スキャン）が行われます。
...
- バンドル内の `.md` ファイルを変更しても `source_hash` は変化しません
  ...
  テンプレートを別の `.md` に分離する場合、モジュール読み込み時にメモリへロードするのは実用上動作しますが、
  resume 時の整合性チェックでは捕捉できません（v2 への持ち越し）。
...
- `review_rounds.py` — Instruction text 内で `--target .claude/skills/agent-sequencer/server` を明示的に渡す
